/**
 * PlanShipment — V3 LangGraph StateGraph + Apollo supergraph fan-out
 *
 * State machine:
 *   idle          → form visible
 *   planning      → loading  (Apollo supergraph fan-out: warehouse+route parallel, carriers, quote)
 *   plan_review   → Gate 1 HLT — human reviews plan; Approve / Reject
 *   creating      → loading  (CreateShipment + BookCarrier run automatically)
 *   dock_review   → Gate 2 HLT — human reviews dock slot; Book / Skip
 *   confirming    → loading  (BookDockSlot if approved)
 *   done          → success card
 *   rejected      → rejection card
 *   error         → error card
 *
 * Backend: POST /api/v3/plan → /api/v3/plan/confirm → /api/v3/dock/confirm
 */

import { useState } from 'react';
import { useQuery }  from '@apollo/client';
import { GET_WAREHOUSES } from '../graphql/queries';
import { planShipmentV3, confirmPlanV3, confirmDockV3 } from '../api/gateway';
...
const fmtC = (n, cur = 'USD') => `${cur} ${fmt(n)}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-blue-50 dark:bg-blue-900/30 border-b border-blue-200 dark:border-blue-800 px-6 py-4 flex items-start gap-3 shrink-0">
          <span className="text-2xl">🔍</span>
          <div>
            <p className="font-semibold text-blue-900 dark:text-blue-200">Gate 1 — Review Shipment Plan</p>
            <p className="text-sm text-blue-700 dark:text-blue-400 mt-0.5">
              Apollo supergraph resolved warehouse, route, carriers and quote.
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Stat label="Est. cost"     value={fmtC(q.totalCost, q.currency)} />
          ...
          {/* Warehouse */}
          <section>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Origin Warehouse</p>
            <CapacityBar cap={cap} />
            <div className="mt-2">
              <InfoTable rows={[
                ['Name',    wh.name],
          ...
          {/* Route */}
          <section>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Recommended Route</p>
            <InfoTable rows={[
              ['Route name',   rec.name],
          ...
          {/* Carrier */}
          <section>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Selected Carrier</p>
            <InfoTable rows={[
              ['Carrier',       car.name],
              ['Code',          car.code],
              ['On-time rate',  `${fmt((perf.onTimeDeliveryRate ?? 0) * 100, 1)}%`],
              ['Avg transit',   `${perf.avgTransitDays ?? '?'} days`],
          ...
          {/* Quote breakdown */}
          <section>
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">Quote Breakdown</p>
            <InfoTable rows={[
              ['Base rate',       fmtC(q.baseRate,      q.currency)],
              ['Fuel surcharge',  fmtC(q.fuelSurcharge, q.currency)],
              ['Handling fee',    fmtC(q.handlingFee,   q.currency)],
              ['Total cost',      fmtC(q.totalCost,     q.currency)],
      ...
      <div className="rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 px-5 py-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-emerald-700 dark:text-emerald-400">
            {result.dockBooked ? 'Shipment created, carrier booked, and dock slot reserved.' : 'Shipment created and carrier booked. Dock slot skipped.'}
          </p>
        </div>
      </div>
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input className="form-input" value={item.sku} onChange={inp('sku')} placeholder="SKU-001" />
        </div>
        <div>
          <label className="form-label">Description</label>
          <input className="form-input" value={item.description} onChange={inp('description')} placeholder="Product name" />
        </div>
        <div>
          <label className="form-label">Qty</label>
          <input className="form-input" type="number" min={1} value={item.quantity} onChange={inp('quantity', true)} />
        </div>
        <div>
        ...
        <div>
          ...
          <input className="form-input" type="number" min={0} step={0.01} value={item.value} onChange={inp('value', true)} />
        </div>
      </div>
...
export default function PlanShipment() {
  ...
  /* flow state */
  const [stage,     setStage]     = useState('idle');   // idle | planning | plan_review | creating | dock_review | confirming | done | rejected | error
  const [threadId,  setThreadId]  = useState(null);
  const [planData,  setPlanData]  = useState(null);    // Gate 1 payload
  const [dockData,  setDockData]  = useState(null);    // Gate 2 payload
  const [result,    setResult]    = useState(null);    // done payload
  ...
  /* ── Phase 1: Get plan (Apollo supergraph fan-out) ──────────────── */
  async function handleSubmit(e) {
    e.preventDefault();
    setStage('planning');
    setError('');
    try {
      const res = await planShipmentV3(body());
      if (res.status === 'error') {
        setError(res.error || 'Planning failed — data missing from supergraph.');
        setStage('error');
        return;
      }
  ...
  /* ── Gate 1: Approve plan → CreateShipment + BookCarrier ─────────── */
  async function handleApprovePlan() {
    setStage('creating');
    try {
      const res = await confirmPlanV3(threadId, true);
      if (res.status === 'error') {
      ...
      if (res.status === 'needs_dock_confirmation') {
        ...
        // Finished without dock gate (shouldn't normally happen)
        setResult(res);
        setStage('done');
      }
    } catch (err) {
  ...
  async function handleRejectPlan() {
    setStage('creating'); // brief loading while graph finishes
    try {
      await confirmPlanV3(threadId, false);
    } catch (_) { /* ignore */ }
    setStage('rejected');
  }

  /* ── Gate 2: Approve / skip dock slot ───────────────────────────── */
  async function handleDock(approved) {
    setStage('confirming');
    try {
      const res = await confirmDockV3(threadId, approved);
      setResult(res);
      setStage('done');
    } catch (err) {
      setError(err.message);
  ...
  /* Loading states */
  const LOADING_MSG = {
    planning:   'Apollo supergraph resolving warehouse, route, carriers and quote…',
    creating:   'LangGraph: creating shipment and booking carrier…',
    confirming: 'LangGraph: finalising dock slot booking…',
  };
  ...
  return (
    <>
      ...
      {/* Gate 2 dialog — dock slot review */}
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            V3 · LangGraph StateGraph · Apollo supergraph fan-out · Two HITL gates
          </p>
        </div>
